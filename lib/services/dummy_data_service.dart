...
class DummyDataService {
  ...
  static Map<String, dynamic>? _rawJsonData; // Store raw JSON for password checking
  ...
  static Future<void> loadDummyData() async {
    ...
    try {
      print('📁 Loading dummy data from assets/data/dummy_users.json...');
      ...
      print('✅ Loaded ${_users!.length} dummy users');
  ...
  static Future<AuthResponse> authenticateUser({
    ...
    try {
      ...
      final matchingUser = usersJson.firstWhere(
            (userJson) =>
        (userJson['username'].toString().toLowerCase() == username.toLowerCase() ||
            userJson['email'].toString().toLowerCase() == username.toLowerCase()) &&
      ...
      if (matchingUser != null) {
        ...
        print('   Expected password from JSON: ${_getPasswordFromJson(username)}');
  ...
  static Future<AuthResponse> createUser({
    ...
    try {
      ...
      if (existingUser != null && existingUser.isNotEmpty) {
        return AuthResponse.failure('User already exists with this username or email');
      }
  ...
  static Future<bool> userExists(String username) async {
    ...
    user.username.toLowerCase() == username.toLowerCase() ||
        user.email.toLowerCase() == username.toLowerCase()) ?? false;
  }
